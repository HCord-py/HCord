...
*hcord* logs errors and debug information via the :mod:`logging` python
...
hcord as well as other libraries that use the ``logging`` module
...
example to write the logs to a file called ``hcord.log`` instead of
outputting them to the console the following snippet can be used::

    import hcord
    import logging

    logger = logging.getLogger('hcord')
    logger.setLevel(logging.DEBUG)
    handler = logging.FileHandler(filename='hcord.log', encoding='utf-8', mode='w')
